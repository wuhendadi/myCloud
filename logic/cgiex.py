...
class FieldStorage:
    ...
    def read_multi(self, environ, keep_blank_values, strict_parsing):
        ...
        while not part.done:
            ...
            part = klass(self.fp, headers, ib,
                         environ, keep_blank_values, strict_parsing, self)
    ...
    bufsize = 16*1024            # I/O buffering size for copy to file
    ...
    def read_lines(self):
        ...
        try:
            ...
            if os.path.exists(self.file.name):
                os.remove(self.file.name.replace("\\","/"))
    ...
    def make_file(self, binary=None):
        ...
        if self.parent and 'path' in self.parent:
            fullpath = unicode(ProfileFunc.slashFormat(self.parent['path'].value))
            ...
            if dname and fname:
                f = tempfile.NamedTemporaryFile('w+b', dir=dname, prefix='.popotmp_', delete=False)
                #f = file(fullpath, 'w+b')
            else:
                f = tempfile.NamedTemporaryFile('w+b', prefix=tmp_prefix, delete=True)
            return f
        else:
            try:
    ...
    def save_to(self, path):
        if hasattr(self.file, 'name'):
            ...
            self.file.seek(0)
            f = open(path, 'wb+')               
            shutil.copyfileobj(fsrc=self.file, fdst=f)
            f.close()
