...
class ResponseFile():
    ...
    def readline(self, length=None):
        ...
        r = self.buf.read(newpos - self.pos)
        self.pos = newpos
        return r
...
def serve_file(path, content_type=None, disposition=None, name=None, debug=False):
    ...
    if disposition is not None:
        ...
        if 'Firefox' in request.headers.get('User-Agent', ""):
            cd = '%s; filename*="%s"' % (disposition, urllib.quote(name))
        else:
...
def _serve_fileobj(fileobj, content_type, content_length, debug=False):
    ...
    if request.protocol >= (1, 1):
        ...
        if r:
            if len(r) == 1:
                ...
                response.body = ResponseFile(fileobj, r_len)
            else:
                ...
                def file_ranges():
                    ...
                    for start, stop in r:
                        ...
                        fileobj.seek(start)
                        chunk = ResponseFile(fileobj, stop-start)
                        yield chunk
                        yield ntob("\r\n")
                    # Final boundary
                    yield ntob("--" + boundary + "--", 'ascii')
    ...
    response.body = ResponseFile(fileobj , content_length) 
    return response.body
