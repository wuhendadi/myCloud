...
def setupPortInfo(portNum,protocol):
    ...
    portInfo['exPort']=portNum+30000+random.randint(0,50)
...
def add_socks_port():
    try:
        portInfo=upnpcAddPortRedirection(setupPortInfo(SOCKS_PORT,'tcp'))
    except Exception,e:
        logger.error(str(e))
        if hasattr(e, 'value'):
            return e.value, None
...
def setup_relay_info():
    ...
    portInfo['port']=SOCKS_PORT
    portInfo['exPort']=SOCKS_PORT
    portInfo['protocol']='tcp'
    return portInfo

def main():
    upnpcAddPortRedirection(setupPortInfo(OPENVPN_PORT,'udp'))
