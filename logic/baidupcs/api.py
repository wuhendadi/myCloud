...
class BufferReader(MultipartEncoder):
    """
    封装MultipartEncoder，支持：
    1.读取块数据前的回调处理。read_callback(read_callback_params, BufferReader)
    """
    def __init__(self, fields, boundary=None, encoding='utf-8', read_callback=None,
                 read_callback_params=None):
        super(BufferReader, self).__init__(fields, boundary, encoding)
        self.__read_callback = read_callback
        self.__read_callback_params = read_callback_params

    def read(self, size=None):
        if self.__read_callback: self.__read_callback(self.__read_callback_params, self)
        return super(BufferReader, self).read(size)
...
class BaseClass(object):
    ...
    def _request(self, uri, method, url=None, extra_params=None,
        ...
        """
        read_callback(read_callback_params, BufferReader): 用来处理停止上传的操作
        """
        params = {
            'method': method,
        ...
        try:
            read_callback = kwargs.pop('read_callback')
            read_callback_params = kwargs.pop('read_callback_params')
        except KeyError, e:
            read_callback = None
            read_callback_params = None
        ...
        if data or files:
            ...
            if data:
                ...
                # To support stopping upload.
                # data = MultipartEncoder(files)
                data = BufferReader(files, read_callback=read_callback,
                                    read_callback_params=read_callback_params)
                if kwargs.get('headers'):
                    kwargs['headers']['Content-Type'] = data.content_type
                else:
                    kwargs['headers'] = {'Content-Type': data.content_type}
                #specify timeout param, for socket will wait for ever
