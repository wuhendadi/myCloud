...
class MultipartEncoder(object):
    ...
    def read(self, size=None):
        ...
        # sometimes,the real bytes to be read is large than size
        written = self._load_bytes(size)

        if written > size:
            return self._buffer.read(written)
        else:
    ...
    def _load_bytes(self, size):
        ...
        # sometimes, there aren't enough space for an end boundary, but data is run out
        # written will be large then specified size
        while size is None or written != size:
            ...
            # shouldn't decrease size here, use another temp var
            if size is not None and written < size:
                #size -= written
                left = size - written
            #written += self._consume_current_data(size)
            written += self._consume_current_data(left)

        self._buffer.seek(orig_position, 0)
        self._buffer.smart_truncate()

        #return the reality bytes to be send
        return written
