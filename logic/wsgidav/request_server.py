...
from dav_error import DAVError, asDAVError, \
    ...
    HTTP_RANGE_NOT_SATISFIABLE, HTTP_INSUFFICIENT_STORAGE
...
#            self._checkWritePermission(res, environ["HTTP_DEPTH"], environ)
    ...
    def doPUT(self, environ, start_response):
        ...
        if not parentRes:
            try:
                parentPath = util.getUriParent(path)
                self._mkdirRecursive(environ, parentPath)
                parentRes = provider.getResourceInst(parentPath, environ)
            except:
                pass
...
#                    assert len(readbuffer) > 0, "input.read(%s) returned %s bytes" % (n, len(readbuffer))
            ...
            if e:
                try:
                    errno = int(e[0])
                    if errno == 28: # [Errno 28] No space left on device
                        storage_full = True
                except:
                    pass
            if not storage_full:
        ...
        if storage_full:
            return util.sendStatusResponse(environ, start_response, HTTP_INSUFFICIENT_STORAGE)
    ...
    def _mkdirRecursive(self, environ, dstPath):
        provider = self._davProvider
        parent = util.getUriParent(dstPath)
        if not provider.exists(parent, environ):
            self._mkdirRecursive(environ, parent)
        parentRes = provider.getResourceInst(parent, environ)
        parentRes.createCollection(util.getUriName(dstPath))
        return
    ...
    def _copyOrMove(self, environ, start_response, isMove):
        ...
        # BUGBUG: direct get the destpath instead of call urlparse()
        index = destinationHeader.find("//")
        if (index >= 0 and index + 2 < len(destinationHeader)):
            tmpPath = destinationHeader[index + 2:]
            index = tmpPath.find("/")
            if index >= 0 and index < len(tmpPath):
                destPath = tmpPath[index:]
        ...
        destParent = util.getUriParent(destPath)
        destParentRes = provider.getResourceInst(destParent, environ)

        if not destParentRes:
            try:
                self._mkdirRecursive(environ, destParent)
                destParentRes = provider.getResourceInst(destParent, environ)
            except:
                pass
